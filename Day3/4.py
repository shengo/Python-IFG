...
for category in listeCategory:			# repertoires pour chaqun categorie
	...
	outTagCategory = outFortag + "/" + category # nom de repertoire por categories de tags
	if not path.exists (outCategory):	# si repertoire existe
		makedirs(outCategory)		# si repertoire n'existe pas

	if not path.exists (outTagCategory):	# si repertoire existe
		makedirs(outTagCategory)		# si repertoire n'existe pas
	counter=0				# variable-compteur por les noms de fichiers

	for i in range (1,4):			# pour 3 page
		...
		for articleurl in blocURLS: 	#pour chaqun URL d'articles
			...
			if article :
				with codecs.open(os.path.join(out , category, nomFichiers) , "w" , encoding="utf-8") as fout:
					fout.write(article)
				fout.close() # finir
				with codecs.open(os.path.join(out, category, nomFichiers),"w",encoding="utf-8") as fouttag:
				#aq davisvenot :P
